...
namespace YoukyController
{
    ...
    public class SPlayerAnimator : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            if (!playerCon.AttackingThisFrame && !UnityEngine.Input.GetMouseButton(0))
            {
            ...
            // 跑步
            anim.SetFloat(SpeedKey, Mathf.Lerp(0, maxIdleSpeed, Mathf.Abs(playerCon.Input.X)));
            if (Mathf.Abs(playerCon.Input.X) < 0.1f ) anim.SetBool("isRun", false);
            else if(playerCon.Grounded) anim.SetBool("isRun", true);
